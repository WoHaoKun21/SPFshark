//
//  QWeatherConfigModel.h
//  QWeather_Plugin
//
//  Created by he on 2019/5/13.
//  Copyright © 2019 QWeatherTianQi. All rights reserved.
//
#define QWeatherColorFromRGB(rgbValue) [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]
...
/**
 ...
 - QWeatherConfigModelTypeWeatherState: 天气状况
 ...
 - QWeatherConfigModelTypeAqiTitle: 空气质量文字AQI
 - QWeatherConfigModelTypeQlty: 空气质量等级
 - QWeatherConfigModelTypeAqi: 空气质量
 */
typedef NS_ENUM(NSInteger, QWeatherConfigModelType) {
...
@interface QWeatherConfigModel : NSObject
...
/**
 空气质量颜色数组只对 QWeatherConfigModelTypeQlty QWeatherConfigModelTypeAqi类型有效 数组按顺序存放颜色，对应的顺序为优、良、轻度、中度、重度、严重， 六个等级都要设置才有效否则使用默认
 */
@property (nonatomic,strong) NSArray *airTextColorArray;

/**
 预警文字颜色数组只对 QWeatherConfigModelTypeAlarm类型有效 数组按顺序存放颜色，对应的顺序为白色、蓝色、黄色、橙色、红色， 五个等级都要设置才有效否则使用默认
 */
...
/**
 控件type
 */
@property (nonatomic,assign) QWeatherConfigModelType type;
